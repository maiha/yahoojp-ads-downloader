...
@assert_file_exists() {
  local file=$1
  if [ ! -f "$file" ]; then
    echo -n $(@colorize $red "File not found: ")
    echo $(@colorize $cyan "$file")
    echo $(@colorize $yellow " [$PWD]")
    exit 1
  fi
}
...
@assert_dir_exists() {
  local dir=$1
  if [ ! -d "$dir" ]; then
    echo -n $(@colorize $red "Directory not found: ")
    echo $(@colorize $cyan "$dir")
    echo $(@colorize $yellow " [$PWD]")
    exit 1
  fi
}
...
@assert_matches() {
  local pattern=$1
  if ! grep -qE "$pattern" run.out; then
    echo -n $(@colorize $red "Output does not match pattern: ")
    echo $(@colorize $cyan "$pattern")
    echo $(@colorize $yellow " [$PWD]")
    echo "Output was:"
    sed -e 's/^/    /' run.out
    exit 1
